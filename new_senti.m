...
clear;
close all;
Data=load('sentiment_processed.mat');
X_test=Data.xTe;
X_train=Data.xTr;
Y_test=Data.yTe;
Y_train=Data.yTr;
T=size(X_train,2);
for t = 1:T
    Y_train{t} = (Y_train{t} -1.5)*2;
    Y_test{t} = (Y_test{t} -1.5)*2;
end
for t = 1:T
    [coeff, score] = pca(X_train{t});
    reducedDimension = coeff(:,1:10);
    X_train{t} = X_train{t} * reducedDimension;
end
[ui,D]=size(X_train{1}); 
K = 2; % No of clusters
...
%%E_Step
    ...
    for t=1:T
        ...
        for k=1:K
            ...
            fprintf('po = %f \n', po);
            mo = mvnpdf(w(t,:),mu_k(k,:),sigma_2(k)*eye(D))
            %fprintf('mo = %f \n', mo)
            Gamma(t,k)= Pi(k)*mo*po;
            fprintf('gamma(%d,%d) = %f \n', t, k, Gamma(t,k));
        end
    ...
    if(mod(l,5) == 0)
        s = int2str(l);
        filename = strcat('Workspace_sp',s);
        save(filename);
    end
end
